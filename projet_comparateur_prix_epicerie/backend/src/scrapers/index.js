const SuperCScraper = require('./superc');
...
async function findOrCreateProduct(productData) {
  ...
  try {
    ...
    // This prevents duplicates better than checking size (which is often empty)
...
/**
 * Scrape all Super C products from all categories
 */
async function scrapeAllSuperC() {
  console.log('\n=== Starting Full Super C Scraping ===\n');

  const scraper = new SuperCScraper();
  let totalProducts = 0;
  let totalPrices = 0;
  ...
  try {
    ...
    // Get or create Super C store
    const storeId = await findOrCreateStore('Super C', 'https://www.superc.ca');
    ...
    for (let i = 0; i < categories.length; i++) {
      ...
      console.log(`\n[${i + 1}/${categories.length}] Scraping category: ${category.name}`);
    ...
    console.log(`\n=== Scraping Complete ===`);
    console.log(`Total categories scraped: ${categories.length}`);
 ...
 */
async function scrapeCategories(categoryNames) {
  console.log('\n=== Starting Selective Super C Scraping ===\n');
  ...
  try {
    ...
    // Get all categories
    const allCategories = await scraper.getAllCategories();

    // Filter to requested categories
    const categories = allCategories.filter(cat =>
      categoryNames.some(name => cat.slug.includes(name.toLowerCase().replace(/\s+/g, '-')))
    );
...
async function main() {
  ...
  if (args.length > 0 && args[0] === '--all') {
    ...
    await scrapeAllSuperC();
  } else if (args.length > 0) {
    // Scrape specific categories
    await scrapeCategories(args);
  } else {
    console.log('Usage:');
    console.log('  node src/scrapers/index.js --all              # Scrape all categories');
    console.log('  node src/scrapers/index.js fruits legumes     # Scrape specific categories');
    console.log('  node src/scrapers/index.js produits-laitiers  # Scrape by slug');
    process.exit(0);
  }
